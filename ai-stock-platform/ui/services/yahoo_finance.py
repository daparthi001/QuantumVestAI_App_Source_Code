...
class YahooFinanceService:
    ...
    def get_technical_indicators(cls, ticker: str, period: str = TIMEFRAME_1Y) -> Dict[str, Any]:
        ...
        try:
            ...
            # Add trend signals
            indicators["signals"] = {
                "ma_trend": "bullish" if latest.get("MA_50", 0) > latest.get("MA_200", 0) else "bearish",
                "rsi_signal": "oversold" if latest.get("RSI", 50) < 30 else "overbought" if latest.get("RSI", 50) > 70 else "neutral",
                "bollinger_position": "upper" if latest.get("close", 0) > latest.get("BB_UPPER", 0) else "lower" if latest.get("close", 0) < latest.get("BB_LOWER", 0) else "middle"
            }
