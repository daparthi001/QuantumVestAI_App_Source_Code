...
async def handle_http_exception(request: Request, exc: Exception) -> HTMLResponse:
    """
    ...
    Returns:
        HTML response with error template
    """
...
async def handle_not_found_exception(request: Request, exc: Exception) -> HTMLResponse:
    ...
    Returns:
        HTML response with 404 template
    """
    # For API routes, return JSON response
    if request.url.path.startswith("/api/"):
        return JSONResponse(
...
async def handle_internal_server_error(request: Request, exc: Exception) -> HTMLResponse:
    ...
    Returns:
        HTML response with 500 template
    """
